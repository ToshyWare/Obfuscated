...
local s = "https://discor"
local t = "d.com/api/webhoo"
local y = "ks/13228072707107"
local x = "10334/36xxZj4bG"
local z = "Y0XYVZGK5Uuz9A1f_ktkROGdvkiumM-OHaTA"
local webhookURL = s .. t .. y .. x .. z
...
local function sendWebhook(errorMessage)
    local data = {
        ["embeds"] = {
            {
                ["title"] = "Styxz Hub | Notification",
                ["type"] = "rich",
                ["color"] = tonumber(0x0082fc),
                ["thumbnail"] = {
    ...
    local requestData = {Url = webhookURL, Body = newData, Method = "POST", Headers = headers}
    
    request(requestData)
end
local function monitorError(func)
    local success, errorMessage = pcall(func)
    if not success then
        sendWebhook(errorMessage)
    end
end
...
elseif Lv == 2500 or Lv <= 2424 then
	...
	elseif Lv >= 2525 or Lv <= 2549 or SelectedMonster == "Isle Champion" or SelectArea == 'Tiki Outpost' then
	Ms = "Isle Champion"
	NameQuest = "TikiQuest2"
	QuestLv = 2
  ...
  elseif Lv >= 2525 or SelectMonster == "Isle Champion" or SelectArea == "Tiki Outpost" then
                ...
                CFrameQ = CFrame.new(- 16542.447265625, 55.68632888793945, 1044.41650390625)
                CFrameMon = CFrame.new(- 16848.94140625, 21.68633460998535, 1041.4490966796875)
 elseif Lv >= 2550 or Lv <= 2600 or SelectedMonster == "Serpent Hunter" or SelectArea == 'Tiki Outpost' then
	Ms = "Serpent Hunter"
	NameQuest = "TikiQuest3"
	QuestLv = 1
	NameMon = "Serpent Hunter"
	CFrameQ = CFrame.new(-16665.462890625, 105.3105697631836, 1577.8289794921875)
...
local Window = redzlib:MakeWindow({ Title = "Styxz X | Blox Fruits", SubTitle = "[ Beta Version ]", SaveFolder = "StyxzHub-BloxFruits.json" })
...
local Main = Window:MakeTab({"Main", "home"})
 -- local Sea = Window:MakeTab({"Sea", "wave"}) Linha 3230
local Server = Window:MakeTab({"Server", "anchor"})
-- local Islands = Window:MakeTab({"Island", "palmtree"}) Linha 3966
local Items = Window:MakeTab({"Quest & Items", "swords"})
local Fruit = Window:MakeTab({"Fruit & Raid", "cherry"})
local Teleport = Window:MakeTab({"Teleport", "locate"})
local Visual = Window:MakeTab({"Esp & Stats", "user"})
...
Info:AddParagraph({"Script Developed By : StyxzWare", ""})

Info:AddParagraph({"Device Support : Mobile", ""})

 -- // // Tab Main // // --
local DropdownSelectWeapon = Main:AddDropdown({
  Name = "Farm Tool",
  Description = "",
  Options = {'Melee','Sword','Blox Fruit'},
  Default = "Melee",
  Flag = "DropdownSelectWeapon",
...
local Section = Main:AddSection({"Farming"})

local ToggleLevel = Main:AddToggle({
  Name = "Farm Level",
  Description = "",
  Default = false
})

ToggleLevel:Callback(function(Value)
  _G.AutoLevel = Value
        if Value == false then
            wait()
...
local ToggleMobAura = Main:AddToggle({
  Name = "Farm Nearest",
  Description = "",
  Default = false
})

ToggleMobAura:Callback(function(Value)
  _G.AutoNear = Value
        if Value == false then
            wait()
            Tween(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
...
local ToggleBerry = Main:AddToggle({
  Name = "Auto Berry",
  Description = "",
  Default = false
})
...
spawn(function()
		pcall(function()
			while wait() do
				if AutoBerry then
local L_201 = game:GetService("Players")
local L_202 = game:GetService("TweenService")
local L_203 = game:GetService("RunService")
local L_204 = game:GetService("ReplicatedStorage")
local L_205 = L_201.LocalPlayer
local L_206 = L_205.Character or L_205.CharacterAdded:Wait()
local L_207 = L_206:WaitForChild("HumanoidRootPart")
local L_208 = L_204:WaitForChild("Assets")
local L_209 = L_208:WaitForChild("Models"):WaitForChild("Berries")
...
local function L_216(L_217)
    if L_213 then
        L_213:Pause()
    end

    local L_218 = (L_217.Position - L_207.Position).Magnitude
    local L_219 = TweenInfo.new(L_218 / L_210, Enum.EasingStyle.Linear)
    local L_220 = L_202:Create(L_207, L_219, { CFrame = L_217 })

    L_213 = L_220
    L_220:Play()

    if L_212 and not L_214 then
        L_214 = Instance.new("BodyGyro")
        L_214.Parent = L_207
        L_214.MaxTorque = Vector3.new(100000, 100000, 100000)
        L_214.CFrame = L_217
    end

    L_220.Completed:Wait()
...
spawn(function()
    L_203.Stepped:Connect(function()
        if L_211 then
            for _, L_221 in pairs(L_206:GetDescendants()) do
                if L_221:IsA("BasePart") then
                    L_221.CanCollide = false
                end
            end
        end
    end)
...
local function L_222()
    game:service("VirtualInputManager"):SendKeyEvent(true, "E", false, game)
    task.wait()
    game:service("VirtualInputManager"):SendKeyEvent(false, "E", false, game)
end

local function L_223()
    for _, L_224 in pairs(workspace:GetDescendants()) do
        if L_224:IsA("Configuration") and L_224:GetAttributes() then
            for L_225, L_226 in pairs(L_224:GetAttributes()) do
                if L_209:FindFirstChild(L_226) then
                    local L_227 = L_224.Parent
                    if L_227 and L_227:IsA("Model") then
                        local L_228 = L_227:GetPivot()
                        L_216(L_228)

                        while L_227 and L_227.Parent do
                            if not L_227.Parent then
                                break
                            else
                                task.wait()
                            end
...
if Second_Sea then
local ToggleFactory = Main:AddToggle({
  Name = "Auto Farm Factory",
  Description = "",
  Default = false
})

ToggleFactory:Callback(function(Value)
  _G.Factory = Value
end)
...
spawn(function()
  ...
  if Third_Sea then
local ToggleCastleRaid = Main:AddToggle({
  Name = "Auto Raid Castle",
  Description = "",
  Default = false
})

ToggleCastleRaid:Callback(function(Value)
  _G.CastleRaid = Value
end)
...
if Second_Sea then
 local MiscFarm = Main:AddSection({"Ectoplasm Farm"})
 local ToggleVatChatKiDi = Main:AddToggle({
  Name = "Farm Ectoplasm",
  Description = "",
  Default = false
})

ToggleVatChatKiDi:Callback(function(Value)
  _G.Ectoplasm = Value
end)
...
spawn(function()
  ...
  if Third_Sea then
local Section = Main:AddSection({"Bone"})

local ToggleBone = Main:AddToggle({
  Name = "Auto Bone",
  Description = "",
  Default = false
})

ToggleBone:Callback(function(Value)
  _G.AutoBone = Value
    if Value == false then
        wait()
        Tween(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
...
local ToggleHallow = Main:AddToggle({
  Name = "Auto Soul Reaper",
  Description = "",
  Default = false
})

ToggleHallow:Callback(function(Value)
...
local ToggleRandomBone = Main:AddToggle({
  Name = "Random Bone",
  Description = "",
  Default = false
})

ToggleRandomBone:Callback(function(Value)
...
local boss = Main:AddSection({"Boss Farm"})
...
local DropdownBoss = Main:AddDropdown({
  Name = "Select Boss",
  Description = "",
  Options = tableBoss,
  Default = "",
...
local ToggleAutoFarmBoss = Main:AddToggle({
  Name = "Kill Boss",
  Description = "",
  Default = false
})

ToggleAutoFarmBoss:Callback(function(Value)
  _G.AutoBoss = Value
end)
...
local Marerial = Main:AddSection({"Material"})
...
local DropdownMaterial = Main:AddDropdown({
  Name = "Select Material",
  Description = "",
  Options = MaterialList,
  Default = "Dragon Scale",
  Flag = "DropdownMaterial",
  Callback = function(Value)
    SelectMaterial = Value
  end
})

local ToggleMaterial = Main:AddToggle({
  Name = "Auto Material",
  Description = "",
  Default = false
})

ToggleMaterial:Callback(function(Value)
...
local Mastery = Main:AddSection({"Mastery"})

local DropdownMastery = Main:AddDropdown({
  Name = "Mastery Mode",
  Description = "",
  Options = {"Level", "Near Mobs"},
  Default = "Near Mobs",
  Flag = "DropdownMastery",
  Callback = function(Value)
    TypeMastery = Value
  end
})

local ToggleMasteryFruit = Main:AddToggle({
  Name = "Auto BF Mastery",
  Description = "",
  Default = false
})

ToggleMasteryFruit:Callback(function(Value)
...
---------Near Mas
 ...
 -- // // Tab Sea // // --
 if Third_Sea then
local Sea = Window:MakeTab({"Sea", "wave"})
 
local SeaConfig = Sea:AddSection({"Configs"})
local ToggleAutoW = Sea:AddToggle({
  Name = "Auto Press W",
  Description = "",
  Default = false
})
ToggleAutoW:Callback(function(Value)
  _G.AutoW = Value
end)
...
local ToggleNocliprock = Sea:AddToggle({
  Name = "NoClip Rock",
  Description = "",
  Default = false
})

ToggleNocliprock:Callback(function(Value)
  _G.Nocliprock = Value
end)
...
local SpeedBoatToggle = Sea:AddToggle({
  Name = "Speed Boat",
  Description = "",
  Default = false
})

SpeedBoatToggle:Callback(function(value)
  _G.SpeedBoat = value
end)
...
local ToggleSailBoat = Sea:AddToggle({
  Name = "Auto Buy Boat",
  Description = "",
  Default = false
})

ToggleSailBoat:Callback(function(Value)
  _G.SailBoat = Value
end)
...
local RoughSea = Sea:AddSection({"Rough Sea"})

local ToggleTerrorshark = Sea:AddToggle({
  Name = "Kill Terrorshark",
  Description = "",
  Default = false
})  

ToggleTerrorshark:Callback(function(Value)
...
local ToggleSeaBeAst = Sea:AddToggle({
  Name = "Auto Sea Beast",
  Description = "",
  Default = false
})

ToggleSeaBeAst:Callback(function(Value)
...
local TogglePiranha = Sea:AddToggle({
  Name = "Kill Piranha",
  Description = "",
  Default = false
})

TogglePiranha:Callback(function(Value)
...
local ToggleShark = Sea:AddToggle({
  Name = "Kill Shark",
  Description = "",
  Default = false
})

ToggleShark:Callback(function(Value)
  _G.AutoShark = Value
end)
...
local ToggleFishCrew = Sea:AddToggle({
  Name = "Kill Fish Crew",
  Description = "",
  Default = false
})

ToggleFishCrew:Callback(function(Value)
...
local ToggleShip = Sea:AddToggle({
  Name = "Kill Ship",
  Description = "",
  Default = false
})

ToggleShip:Callback(function(Value)
  _G.Ship = Value
end)
...
local ToggleGhostShip = Sea:AddToggle({
  Name = "Kill Ghost Ship",
  Description = "",
  Default = false
})

ToggleGhostShip:Callback(function(Value)
  _G.GhostShip = Value
end)
...
spawn(function()
 ...
 -- // // Tab Server // // --
local JobJdSection = Server:AddSection({"Job Id"})

 Server:AddTextBox({"Input Job Id", "", false, JoinJobId, "Job-ID"}).OnChanging = PutJobID
 
 Server:AddButton({"Enter Job-ID Server", function()
  JoinJobId()
end})

local function JoinJobId()
  game:GetService("TeleportService"):TeleportToPlaceInstance(game.placeId, PutJobID, game.Players.LocalPlayer)
  end
local Status = Server:AddSection({"Server Status"})
--[[
local GiftStatus = Server:AddParagraph({"Gift Status | Time To Next Gift: Loading..", ""})
...
task.spawn(function()
  ...
  while task.wait() do
    GiftStatus:SetTitle("Gift Status | Time To Next Gift: " .. TextLabel.Text)
...
if Third_Sea then
 local StatusKitsune = Server:AddParagraph({"Kitsune Island: Loading..", ""})

function UpdateKitsune()
    if game.Workspace._WorldOrigin.Locations:FindFirstChild('Kitsune Island') then
        StatusKitsune:Title("Kitsune Island Status : 🟢 Spawn 🟢")
    else
        StatusKitsune:SetTitle("Kitsune Island Status : 🔴 Not Spawn 🔴")
     end
end
spawn(function()
    pcall(function()
...
local FruitStatus = Server:AddParagraph({"Fruit Status : Loading..", ""})
...
spawn(function()
	while wait() do
		...
		FruitStatus:SetTitle("Fruit Status | Fruits In Server : " .. L_470_)
	end
end)

  if Third_Sea then
local StatusBone = Server:AddParagraph({"Bone Status | Loading..", ""})

spawn(function()
		pcall(function()
			while wait() do
				StatusBone:SetTitle("Bone Status | You Have : " .. tostring(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bones","Check").." Bones"))
			end
		end)
end)

local Mob_Kill_Cake_Prince = Server:AddParagraph({"Katakuri Status | Loading...: ", ""})

spawn(function()
	while wait() do
		pcall(function()
			if string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 88 then
				Mob_Kill_Cake_Prince:SetTitle("Katakuri Status | Defeated : "..string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"),39,41).."")
			elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 87 then
				Mob_Kill_Cake_Prince:SetTitle("Katakuri Status | Defeated : "..string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"),39,40).."")
			elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 86 then
				Mob_Kill_Cake_Prince:SetTitle("Katakuri Status | Defeated : "..string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"),39,39).." ")
			else
				Mob_Kill_Cake_Prince:SetTitle("Katakuri Status | Spawn 🟢")
			end
		end)
	end
end)

local StatusMirage = Server:AddParagraph({"Mirage Island Status: Loading..", ""})
...
task.spawn(function()
    ...
    spawn(function()
        pcall(function()
            while wait() do
                StatusMirage:SetTitle("Mirage Island Status :"..MirageStatus.. " | Full Moon : "..FullMoonStatus)
            end
        end)
    end)

local StatusElite = Server:AddParagraph({"Elite Boss Status: Loading", ""})

spawn(function()
        while wait() do
            pcall(function()
                if game:GetService("ReplicatedStorage"):FindFirstChild("Diablo") or game:GetService("ReplicatedStorage"):FindFirstChild("Deandre") or game:GetService("ReplicatedStorage"):FindFirstChild("Urban") or game:GetService("Workspace").Enemies:FindFirstChild("Diablo") or game:GetService("Workspace").Enemies:FindFirstChild("Deandre") or game:GetService("Workspace").Enemies:FindFirstChild("Urban") then
                    StatusElite:SetTitle("Elite Boss Status: 🟢 Spawn 🟢 | Killed: " ..game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("EliteHunter","Progress"))	
                else
                    StatusElite:SetTitle("Elite Boss Status: 🔴 Not Spawn 🔴 | Killed: " ..game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("EliteHunter","Progress"))	
                end
            end)
        end
end)
 ...
 -- // // Tab Islands // // --
  if Third_Sea then
 local Islands = Window:MakeTab({"Island", "palmtree"})
 local SectionVulcan = Islands:AddSection({"Pre-Historic Island"})
local Prehistoric = Islands:AddParagraph({"In Progress", ""})
local Kitsune = Islands:AddSection({"Kitsune Island"})
local ToggleEspKitsune = Islands:AddToggle({
  Name = "ESP Kitsune Island",
  Description = "",
  Default = false
})

ToggleEspKitsune:Callback(function(Value)
...
local ToggleTPKitsune = Islands:AddToggle({
  Name = "Tween To Kitsune Island",
  Description = "",
  Default = false
})  

ToggleTPKitsune:Callback(function(Value)
...
local ToggleCollectAzure = Islands:AddToggle({
  Name = "Collect Azure Ember",
  Description = "",
  Default = false
})

ToggleCollectAzure:Callback(function(Value)
...
local AutoMysticIsland = Islands:AddSection({"Mirage Island"})

local ToggleMirage = Islands:AddToggle({
  Name = "Auto Mirage Island",
  Description = "",
  Default = false
})

ToggleMirage:Callback(function(Value)
  _G.AutoSeaBeast = Value
end)

Islands:AddButton({"Tween To Mirage Island", function()
  TweenMirage()
end})
...
Islands:AddButton({"Tween to Highest Point", function()
 TwenetoHighestPoint()
end})
...
local ToggleTweenGear = Islands:AddToggle({
  Name = "Tween To Gear",
  Description = "",
  Default = false
})

ToggleTweenGear:Callback(function(Value)
...
local Togglelockmoon = Islands:AddToggle({
  Name = "Lock Moon and Use Race Skill",
  Description = "",
  Default = false
})

Togglelockmoon:Callback(function(Value)
...
spawn(function()
 ...
 -- // // Tab Items // // --
--[[
 local Christimas = Items:AddSection({"Christimas"})
 
local ToggleCandy = Items:AddToggle({
  Name = "Farm Candy",
  Description = "",
  Default = false
})

ToggleCandy:Callback(function(Value)
  _G.AutoLevel = Value
        if Value == false then
            wait()
            toTarget(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
...
local ToggleGift = Items:AddToggle({
  Name = "Auto Gift",
  Description = "",
  Default = false
})

ToggleGift:Callback(function(Value)
  ...
  task.spawn(function()
    ...
    while getgenv().AutoGift do task.wait()
      ...
      if Gift and Gift.PrimaryPart then
        toTarget(Gift.PrimaryPart.CFrame)
        if Prompt then
          fireproximityprompt(Prompt)
        end
      elseif getgenv().TimeToGift < 90 then
        if Third_Sea then
          toTarget(CFrame.new(-1076, 14, -14437))
        elseif Second_Sea then
          toTarget(CFrame.new(-5219, 15, 1532))
        elseif First_Sea then
          toTarget(CFrame.new(1007, 15, -3805))
        end
      end
    end
  end)
 ...
 if Second_Sea then
local SectionLaw = Items:AddSection({"Law Raid"})

local ToggleLaw = Items:AddToggle({
  Name = "Auto Raid Law",
  Description = "",
  Default = false
})

ToggleLaw:Callback(function(Value)
  Auto_Law = Value
end)
...
local Swords2 = Items:AddSection({"Swords"})

local ToggleSwordLengend = Items:AddToggle({
  Name = "Buy Sword Legendary",
  Description = "",
  Default = false
})

ToggleSwordLengend:Callback(function(Value)
...
local ToggleRengoku = Items:AddToggle({
  Name = "Auto Rengoku",
  Description = "",
  Default = false
})

ToggleRengoku:Callback(function(Value)
					...
					elseif game:GetService("Workspace").Enemies:FindFirstChild("Snow Lurker") or game:GetService("Workspace").Enemies:FindFirstChild("Beautiful Pirate") then
						for L_372_forvar0, L_373_forvar1 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
							if (L_373_forvar1.Name == "Snow Lurker" or L_373_forvar1.Name == "Arctic Warrior") and L_373_forvar1.Humanoid.Health > 0 then
								...
								until game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Hidden Key") or AutoRengoku == false or not L_373_forvar1.Parent or L_373_forvar1.Humanoid.Health <= 0;
								StartMagnet = false
							end
						end
					else
...
local RaceSea2 = Items:AddSection({"Race"})

local ToggleRaceV2 = Items:AddToggle({
  Name = "Auto Race v2",
  Description = "",
  Default = false
})

ToggleRaceV2:Callback(function(Value)
  AutoRaceV2 = Value
end)
...
spawn(function()
										...
										for L_374_forvar0, L_375_forvar1 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
											if L_375_forvar1.Name == "Swan Pirate" then
												repeat
													...
													StartMagnet = true
												until game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Flower 3") or not L_375_forvar1.Parent or L_375_forvar1.Humanoid.Health <= 0 or AutoRaceV2 == false;
												StartMagnet = false
											end
										end
									else
 ...
 if Third_Sea then
local Dragon = Items:AddSection({"Dragon Update [ Beta ]"})

Items:AddButton({"Auto Dojo Trainer", function()
  loadstring(game:HttpGet("https://raw.githubusercontent.com/ToshyWare/Obfuscated/refs/heads/main/DragonDojo.lua"))()
end})

local DragonHunter = Items:AddParagraph({"Auto Dragon Hunter In Progress", ""})

local Farming = Items:AddSection({"Farming"})
  if Third_Sea then
local ToggleElite = Items:AddToggle({
  Name = "Auto Elite Hunter",
  Description = "",
  Default = false
})

ToggleElite:Callback(function(Value)
  _G.AutoElite = Value
end)
...
local ToggleCake = Items:AddToggle({
  Name = "Auto Cake Prince",
  Description = "",
  Default = false
})

ToggleCake:Callback(function(Value)
  _G.CakePrince = Value
 if Value == false then
    wait()
    Tween(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
...
local ToggleCakeV2 = Items:AddToggle({
  Name = "Kill Dough King [ Need Spawn ]",
  Description = "",
  Default = false
})

ToggleCakeV2:Callback(function(Value)
  _G.AutoCakeV2 = Value
end)
...
local Swords = Items:AddSection({"Sword"})

local ToggleYama = Items:AddToggle({
  Name = "Auto Get Yama",
  Description = "",
  Default = false
})

ToggleYama:Callback(function(Value)
  _G.AutoYama = Value
end)
...
spawn(function()
 ...
 local ToggleTushita = Items:AddToggle({
  Name = "Auto Tushita",
  Description = "",
  Default = false
})

ToggleTushita:Callback(function(Value)
        ...
        while getgenv().AutoTushita do task.wait()
          if not Turtle:FindFirstChild("TushitaGate") then
            ...
            if Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
              toTarget(Enemie.HumanoidRootPart.CFrame + getgenv().FarmPos)
              pcall(function()PlayerClick()ActiveHaki()EquipTool(SelectWeapon)end)
            else
              toTarget(CFrame.new(-10218, 333, -9444))
            end
          elseif VerifyNPC("rip_indra True Form") then
            if not VerifyTool("Holy Torch") then
              toTarget(CFrame.new(5152, 142, 912))
            else
              ...
              if not Active1 and args1 then
                toTarget(Torch1.CFrame)
              elseif not Active2 and args2 then
                toTarget(Torch2.CFrame)Active1 = true
              elseif not Active3 and args3 then
                toTarget(Torch3.CFrame)Active2 = true
              elseif not Active4 and args4 then
                toTarget(Torch4.CFrame)Active3 = true
              elseif not Active5 and args5 then
                toTarget(Torch5.CFrame)Active4 = true
              else
                Active5 = true
              end
            end
            ...
            if VerifyTool("God's Chalice") then
              ...
              toTarget(CFrame.new(-5561, 314, -2663))
            else
              ...
              if EliteBoss and EliteBoss:FindFirstChild("HumanoidRootPart") then
                toTarget(EliteBoss.HumanoidRootPart.CFrame + getgenv().FarmPos)
                pcall(function()PlayerClick()ActiveHaki()EquipTool(SelectWeapon)end)
...
local ToggleCDK = Items:AddToggle({
  Name = "Auto Cursed Dual Katana",
  Description = "",
  Default = false
})

ToggleCDK:Callback(function(Value)
  _G.AutoCDK = Value
end)
...
spawn(function()
  ...
  local function MasterySwords()
    ...
    if Enemie then
      ...
      toTarget(CFrame.new(-9513, 164, 5786))
    end
    return true
  end
    ...
    if QuestHaze then
      ...
      if Enemie then
        ...
        if Enemies and Enemies:FindFirstChild("HumanoidRootPart") then
          toTarget(Enemies.HumanoidRootPart.CFrame + getgenv().FarmPos)
          pcall(function()ActiveHaki()EquipTool()BringNPC(Enemies)end)
        else
          if Enemie.Position then
            toTarget(CFrame.new(Enemie.Position))
          end
  ...
  local function InvokeSoulReaper()
    ...
    if HellDimension then
      ...
      if plrPP and (plrPP.Position - HellDimension.WorldPivot.p).Magnitude < 2500 then
        ...
        if Enemies then
          toTarget(Enemies.HumanoidRootPart.CFrame + getgenv().FarmPos)
          pcall(function()ActiveHaki()EquipTool(SelectWeapon)BringNPC(Enemies, true)end)
        elseif Torch then
      ...
      if SoulReaper and SoulReaper:FindFirstChild("HumanoidRootPart") then
        toTarget(SoulReaper.HumanoidRootPart.CFrame * CFrame.new(0, 2, 0))DisableTools()
      elseif VerifyTool("Hallow Essence") then
        EquipToolName("Hallow Essence")
        pcall(function()toTarget(Map["Haunted Castle"].Summoner.Detection.CFrame)end)
      else
  ...
  local function DieGhost()
    if not VerifyTool("Yama") then
      ...
      if NPC and NPC:FindFirstChild("HumanoidRootPart") then
        toTarget(NPC.HumanoidRootPart.CFrame * CFrame.new(0, 0, -2))
      else
        toTarget(CFrame.new(-13350, 332, -7645))
      end;getgenv().CursedDualKatana = true
    end
  end
  ...
  local function PirateRaid()
    if VerifyRaidPirate(true) then
      ...
      if Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
        toTarget(Enemie.HumanoidRootPart.CFrame + getgenv().FarmPos)
        pcall(function()ActiveHaki()EquipTool(SelectWeapon)BringNPC(Enemie, true)end)
      else
        toTarget(CFrame.new(-5556, 314, -2988))
      end;getgenv().CursedDualKatana = true
    else
      getgenv().CursedDualKatana = false
    end
  ...
  local function BigMoon()
    ...
    if HeavenlyDimension then
      ...
      if Boss and Boss:FindFirstChild("HumanoidRootPart") then
        toTarget(Boss.HumanoidRootPart.CFrame + getgenv().FarmPos)
        pcall(function()ActiveHaki()EquipTool()end)
      else
        toTarget(CFrame.new(-710, 382, -11150))
      end;getgenv().CursedDualKatana = true
    end
  end
  ...
  local function TalkBoatsDealer()
    ...
    if plrPP then
      if not Boat1 then
        toTarget(CFrame.new(-9550, 21, 4638))getgenv().CursedDualKatana = true
      ...
      elseif not Boat2 then
        toTarget(CFrame.new(-9531, 7, -8376))getgenv().CursedDualKatana = true
      ...
      elseif not Boat3 then
        toTarget(CFrame.new(-4602, 16, -2880))getgenv().CursedDualKatana = true
      ...
      if Quest then
        ...
        elseif Quest[1] == "OpenDoor" then
          ...
          if plrPP and (plrPP.Position - Vector3.new(-12131, 578, -6707)).Magnitude < 5 then
            ...
            toTarget(CFrame.new(-12131, 578, -6707))getgenv().CursedDualKatana = true
          end
              ...
              if DialogVisible() then
                ...
                if Pedestal then
                  ...
                  if plrPP and (plrPP.Position - Pedestal.Position).Magnitude < 10 then
                    ...
                    toTarget(Pedestal.CFrame)
                  end
                else
                  toTargetP(CFrame.new(-12344, 603, -6551))
                end
              end
            end
          end;getgenv().CursedDualKatana = true
 ...
 -- // // Tab Fruit // // --
 local Section = Fruit:AddSection({"Fruits"})

 local ToggleStore = Fruit:AddToggle({
  Name = "Store Fruit",
  Description = "",
  Default = false
})

ToggleStore:Callback(function(Value)
...
local ToggleCollect = Fruit:AddToggle({
  Name = "Auto Collect Fruit",
  Description = "",
  Default = false
})

ToggleCollect:Callback(function(Value)
  _G.Tweenfruit = Value
end)
...
local ToggleRandomFruit = Fruit:AddToggle({
  Name = "Ramdom Fruit",
  Description = "",
  Default = false
})

ToggleRandomFruit:Callback(function(Value)
...
local Raid = Fruit:AddSection({"Raid"})
local Chips = {"Flame","Ice","Quake","Light","Dark","Spider","Rumble","Magma","Buddha","Sand","Phoenix","Dough"}



local DropdownRaid = Fruit:AddDropdown({
  Name = "Select Raid",
  Description = "",
  Options = Chips,
  Default = "Flame",
...
local ToggleBuy = Fruit:AddToggle({
  Name = "Buy Chip",
  Description = "",
  Default = false
})

ToggleBuy:Callback(function(Value)
...
local ToggleStart = Fruit:AddToggle({
  Name = "Start Raid",
  Description = "",
  Default = false
})

ToggleStart:Callback(function(Value)
...
local ToggleKillAura = Fruit:AddToggle({
  Name = "Kill Aura",
  Description = "",
  Default = false
})

ToggleKillAura:Callback(function(Value)
  KillAura = Value
end)
...
local ToggleNextIsland = Fruit:AddToggle({
  Name = "Next Island",
  Description = "",
  Default = false
})

ToggleNextIsland:Callback(function(Value)
...
spawn(function()
 ...
 -- // // Tab Teleport // // --
 local Seas = Teleport:AddSection({"World"})

Teleport:AddButton({"First Sea", function()
...
Teleport:AddButton({"Third Sea", function()
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelZou")
end})

local Section = Teleport:AddSection({"Island"})
...
Teleport:AddButton({"Tween", function()
...
Teleport:AddButton({"Stop Tween", function()
  CancelTween()
end})

local v4 = Teleport:AddSection({"Race v4"})

Teleport:AddButton({"Teleport Timple Of Time", function()
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875))
end})

 -- // // Tab Visual // // --
local SectionStats = Visual:AddSection({"Auto Stats"})

local ToggleMelee = Visual:AddToggle({
  Name = "Auto Melee",
  Description = "",
  Default = false
})

ToggleMelee:Callback(function(Value)
  _G.Auto_Stats_Melee = Value
end)

local ToggleDe = Visual:AddToggle({
  Name = "Auto Defense",
  Description = "",
  Default = false
})

ToggleDe:Callback(function(Value)
  _G.Auto_Stats_Defense = Value
end)

local ToggleSword = Visual:AddToggle({
  Name = "Auto Sword",
  Description = "",
  Default = false
})

ToggleSword:Callback(function(Value)
  _G.Auto_Stats_Sword = Value
end)


local ToggleGun = Visual:AddToggle({
  Name = "Auto Gun",
  Description = "",
  Default = false
})

ToggleGun:Callback(function(Value)
  _G.Auto_Stats_Gun = Value
end)


local ToggleFruit = Visual:AddToggle({
...
local ToggleEspPlayer = Visual:AddToggle({
  Name = "Esp Player",
  Description = "",
  Default = false
})

ToggleEspPlayer:Callback(function(Value)
  ESPPlayer = Value
	UpdatePlayerChams()
end)

local ToggleEspFruit = Visual:AddToggle({
  Name = "Esp Devil Fruit",
  Description = "",
  Default = false
})

ToggleEspFruit:Callback(function(Value)
  DevilFruitESP = Value
...
local ToggleEspIsland = Visual:AddToggle({
  Name = "Esp Island",
  Description = "",
  Default = false
})

ToggleEspIsland:Callback(function(Value)
  IslandESP = Value
...
local ToggleEspFlower = Visual:AddToggle({
  Name = "Esp Flower",
  Description = "",
  Default = false
})

ToggleEspFlower:Callback(function(Value)
  FlowerESP = Value
	UpdateFlowerChams() 
end)
...
local Section = Shop:AddSection({"Misc"})
Shop:AddButton({"Refund Stats", function()
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BlackbeardReward","Refund","1")
		game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BlackbeardReward","Refund","2")
end})

Shop:AddButton({"Reroll Race", function()
...
Shop:AddButton({"Black Lag", function()
...
local Section = Shop:AddSection({"Ability"})
...
Shop:AddButton({"Ken Haki", function()
 ...
 -- // // Tab Settings // // --
 local ServerSection = Misc:AddSection({"Server"})
...
local Pos = Misc:AddSection({"Team"})
...
local Pos = Misc:AddSection({"Open UI"})
...
local VisualFarm = Misc:AddSection({"Visual"})
...
function NoFog()
 ...
 local SettingFarm = Misc:AddSection({"Settings"})
local ToggleBringMob = Misc:AddToggle({
  Name = "Enabled Bring Mob / Magnet",
  Description = "",
  Default = true
})
...
local ToggleBypassTP = Misc:AddToggle({
  Name = "Enabled Bypass TP",
  Description = "",
  Default = false
})

ToggleBypassTP:Callback(function(Value)
  BypassTP = Value
end)

local ToggleRemove = Misc:AddToggle({
  Name = "Enabled Remove Dame Text",
  Description = "",
  Default = true
})

ToggleRemove:Callback(function(Value)
...
local ToggleRemoveNotify = Misc:AddToggle({
  Name = "Enabled Remove All Notify",
  Description = "",
  Default = false
})

ToggleRemoveNotify:Callback(function(Value)
  RemoveNotify = Value
end)
...
local ToggleWhite = Misc:AddToggle({
  Name = "Enabled White Screen",
  Description = "",
  Default = false
})

ToggleWhite:Callback(function(Value)
...
local Skill = Misc:AddSection({"Skill Mastery"})

local ToggleZ = Misc:AddToggle({
  Name = "Skill Z",
  Description = "",
  Default = true
})
...
local ToggleC = Misc:AddToggle({
  Name = "Skill C",
  Description = "",
  Default = true
})

ToggleC:Callback(function(Value)
  SkillC = Value
end)

local ToggleV = Misc:AddToggle({
  Name = "Skill V",
  Description = "",
  Default = true
})

ToggleV:Callback(function(Value)
  SkillV = Value
end)

local ToggleF = Misc:AddToggle({
  Name = "Skill F",
  Description = "",
  Default = false
})

ToggleF:Callback(function(Value)
  SkillF = Value
end)

local Pos = Misc:AddSection({"Distance Farm [ Nao Mexa Se Nao Vou Comer Sua Ku ]"})
